...
const DashBoardCard = lazy(() =>
  import("component/Common/Cards/DashboardCard/DashBoardCard")
);
...
export default function Analytics() {
  ...
  useEffect(() => {
    dataActionCreator.getAdminData(
      dispatch,
      getDashboards,
      dataActions.setLandingAnalytics
    );
  }, [dispatch]);

  useEffect(() => {
    if (data.analyticsLandingData !== undefined) {
      setDashboard(data.analyticsLandingData.data[0]);
    }
  }, [data]);
        ...
        <section className="landing-charts">
          {onboardingMetrics ? (
            <section className="landing-bar">
              <h5 className="graphTitle">
                Interviewers vs Students Onboarding
              </h5>
              <DashBoardChart
          ...
          {trendingData ? (
            <section className="landing-doughnut">
              <h5 className="graphTitle">Trending Topics</h5>
              <DoughNutChart data={trendingData} options={PieChartOptions} />
            </section>
          ) : (
            <p>Loading..</p>
          )}
