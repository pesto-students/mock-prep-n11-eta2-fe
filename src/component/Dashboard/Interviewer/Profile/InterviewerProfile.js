...
export const InterviewerProfile = () => {
  ...
  if (Object.keys(interviewer).length > 0) {
    Object.keys(interviewerForm).forEach(
      (key) => (interviewerForm[key] = interviewer[key])
    );
  }
  ...
  const component1 = (
    <button
      className="btn-interviewer btn btn-success"
      onClick={() => {
        setUpdate(false);
      }}
    >
  ...
  const component2 = (
    <button
      className="btn-interviewer btn btn-primary"
      onClick={() => {
        setUpdate(true);
      }}
    >
