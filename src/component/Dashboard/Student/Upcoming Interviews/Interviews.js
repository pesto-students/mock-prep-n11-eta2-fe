...
export default function Interviews() {
  ...
  useEffect(() => {
    if (data.interviews && auth.user) {
      setInterviews(
        data.interviews.data.filter((e) => e.student.id === auth.user.id)
      );
      setInterviewsList(
        data.interviews.data.filter((e) => e.student.id === auth.user.id)
      );
  ...
  const onSearch = (value) => {
    let filtered = interviewsList.filter(
      (val) => val.student.name.includes(value) || val.topic.includes(value)
    );
  ...
  const search = (
    <>
      <section className="search" id="search1">
        <Search
          placeholder="Search Interviews by any parameter"
          onSearch={onSearch}
          style={{ width: 200 }}
        />
      </section>
  ...
  const search2 = (
    <>
      <section className="search" id="search2">
        <Search
          placeholder="Search Interviews by any parameter"
          onSearch={onSearch}
          style={{ width: 200 }}
