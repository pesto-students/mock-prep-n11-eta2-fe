...
import { getInterviewerById, updateStudent, insertInterviews } from "constant/apiUrl";
...
export default function CalendlyApp  ()  {
  ...
  useEffect(() => { dataActionCreator.getAdminData(dispatch, getInterviewerById + interviewerId, dataActions.setInterviewer) }, [dispatch])

  const handleBooking = async () => {

   
    if ( data.student.data.package && data.student.data.package.interviewsLeft > 0) {
  ...
  function createInterviews() {
    ...
    if (auth && auth.user && data && data.interviewer) { 
      ...
      let options =
      {
          ...
          "date": "01/10/22",
          "time": "15:30",
          "meetingUrl": "https://calendly.com/events/5f49bcd3-331b-4560-93a7-cc4f78244acf/google_meet",
          "status": "new",
          "key": 1,
      }
  ...
  const bookingCompleted = <><button onClick={handleBooking} className="btn btn-primary">Click to Complete Booking</button></>

  return (
    <>
      <DashboardHeader title="Book Interview Slot" icon={userAddIcon} rightComponent={bookingCompleted} />
