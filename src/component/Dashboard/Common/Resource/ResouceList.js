...
import {  getResources, getTopics, insertResource, updateResource,daleteResources } from 'constant/apiUrl'
...
export default function ResourceList() {
    ...
    const handleEdit = (resourceId) => { 
        ...
        if (Object.keys(resourceForm).length > 0) { 
            Object.keys(resourceForm).forEach(key => resourceForm[key] = res[0][key]);
        }
        setShowModal(true)
    }
    ...
    const form = <TopicsForm submitFunction={submit} formFields={resourceForm} buttonValue={"Update Resource"} />
    const form2 = <TopicsForm submitFunction={submit2} topic={topic} formFields={resourceForm} buttonValue={"Add Resource"} />
    ...
    return (
        <div>  
            <section className='resource-container'>
                ...
                <Modals title={"Update Resource"} show={showModal} onHide={() => { setShowModal(false) }} data={form} />
                <Modals title={"Add Resource"} show={showModal2} onHide={() => { setShowModal2(false) }} data={form2}   />
